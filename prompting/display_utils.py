...
def save_episode_html(
    ...
    step_dirs = natsorted(
        glob(
            ...
            os.path.join(episode_path, "step_*")
        )
    )
    ...
    for step, step_dir in enumerate(step_dirs):
        json_files = natsorted(
            glob(
                ...
                os.path.join(step_dir, "prompts", "*.json")
            )
        )
    ...
    js_fname = os.path.join(episode_path, "script.js")
    ...
    html_code = compose_html_file(
        ...
        js_fname=os.path.split(js_fname)[-1],
        html_blocks=all_html_blocks,
        css_code=CSS_CODE,
    )
    # html_fname = f"{episode_path}/{html_fname}.html"
    html_fname = os.path.join(episode_path, f"{html_fname}.html")
